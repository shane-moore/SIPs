...
At Gloas slots operators SHOULD call `produceBlockV4` with `include_payload=true`: a self-build response is then `BlockContents`, so the operator holds the block, envelope, blobs, and proofs from the produce call onward and no beacon-node call sits between the [§4](#4-modified-proposer-duty) decision and the dissemination. Fetching the envelope afterwards instead binds the duty to the node that produced the block, since the envelope endpoint answers only there and the publish rejects a body produced elsewhere. External-build responses are unaffected. After the [§4](#4-modified-proposer-duty) decision, an operator is the builder operator iff the block in its own produce response has the decided block root, the same local test [§4](#4-modified-proposer-duty) uses for the `Eth-Builder-Url` echo, which keeps it correct under round change. Operators sharing a beacon node may both match; they disseminate identical content and both publish, which is harmless. Every other operator disseminates and publishes nothing, and only validates and signs. Blobs and KZG proofs never cross the SSV wire.
