...
At Gloas slots operators SHOULD call `produceBlockV4` with `include_payload=true`, so a self-build response is `BlockContents` and the operator holds the block, envelope, blobs, and proofs from the produce call onward; external-build responses are unaffected. After the [§4](#4-modified-proposer-duty) decision, an operator is the builder operator iff the block in its own produce response has the decided block root, the same local test [§4](#4-modified-proposer-duty) uses for the `Eth-Builder-Url` echo, which keeps it correct under round change. Operators sharing a beacon node may both match; they disseminate identical content and both publish, which is harmless. Every other operator disseminates and publishes nothing, and only validates and signs. The builder operator blinds the envelope from its own `BlockContents`, so no beacon-node call sits between the decision and the dissemination. Blobs and KZG proofs never cross the SSV wire.
...
After the trigger, the builder operator broadcasts one `EnvelopeDissemination` carrying the blinded form of its full envelope. The builder operator SHOULD be the only originator, but receivers cannot verify authorship (the [§4](#4-modified-proposer-duty) block is public, and any operator can claim to have built it), so admission is by content. [§7](#7-ssv-message-validation) admits at most one dissemination per signer, so an operator may hold several. It signs the first whose envelope satisfies every check below against its own [§4](#4-modified-proposer-duty) decision, skipping any that fail, and MUST keep considering later arrivals until the payload-due cutoff, abstaining only if none passes. Every check binds the envelope to the decided block and needs no payload bytes:
...
Each operator signs its selected blinded envelope's progressive signing root under `DOMAIN_BEACON_BUILDER` (`0x0B000000`), domain epoch = `compute_epoch_at_slot(duty.slot)` (matching `verify_execution_payload_envelope_signature`'s `get_domain(state, DOMAIN_BEACON_BUILDER)` against the proposal-slot state); by root-equivalence this is the full envelope's signing root. Partial signatures broadcast as `EnvelopePartialSig`, and each operator validates incoming partials against the root of its own selected envelope, the same expected-root discipline as [§5](#5-proposer-preferences-duty). A partial signature arriving before its recipient has selected an envelope is retried rather than dropped, within the [§7](#7-ssv-message-validation) lateness window.

#### Publication

All operators reconstruct the signature over the selected blinded envelope. Only the operator whose local full envelope blinds to that value can form a valid publish body; every other envelope has a different signing root and fails signature validation.

Publication follows [beacon-APIs #624](https://github.com/ethereum/beacon-APIs/pull/624): `Eth-Blob-Data-Included: true` carries `SignedExecutionPayloadEnvelopeContents`, which the builder operator SHOULD publish to each of its beacon nodes, giving the reveal the same all-node redundancy as the [§4](#4-modified-proposer-duty) block publish. No public `SignedBlindedExecutionPayloadEnvelope` is used.
...
- **EnvelopeProposer (9).**
  - *Dissemination class:* `EnvelopeDissemination` messages share the row's slot, lateness, assignment, and duty-limit rules. Structural rules: exactly one signer (REJECT above); SSZ decode succeeds for the carrier and for its envelope field as the [§6](#6-new-duty-envelope-signing-self-build-path) blinded container, within the SSV message data size cap (REJECT otherwise), so undecodable bytes cannot consume the signer's dissemination budget. Dedup is one per signer: the first message per (`MessageID`, signer, slot) passing all other rules is eligible for ACCEPT; a message that receives ACCEPT MUST be recorded, and implementations MUST then classify further dissemination messages for that tuple as IGNORE, regardless of content or peer. Recording MUST follow the signer-signature check, otherwise a forged carrier claiming another operator's identity consumes that operator's budget and suppresses its honest message. Whether a message that is both a repeat and badly signed is IGNORE'd on this rule or REJECT'd on its signature is left to the implementation, since both verdicts forward nothing and the difference is local peer scoring. Forwarding for the slot is bounded by committee size rather than by one. The [§6](#6-new-duty-envelope-signing-self-build-path) checks binding the envelope to the decided block are runner concerns, not validation rules.
