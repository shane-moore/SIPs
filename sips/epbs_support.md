...
- **Envelope signing is dissemination plus threshold signing, not consensus.** Once [§4](#4-modified-proposer-duty) decides, `bid.block_hash` pins exactly one valid envelope, so there is nothing to negotiate: a consensus round would add message phases inside the tightest window in this SIP and a leader schedule misaligned with the only operator able to originate the envelope. [§6](#6-new-duty-envelope-signing-self-build-path) instead disseminates a `BlindedExecutionPayloadEnvelope` (`payload` → `payload_root: Root`), whose progressive hash tree root equals the full envelope's, and threshold-signs its root. Blinding keeps the disseminated message bounded (~few hundred bytes vs hundreds of KB to ~MB).
...
| `RoleEnvelopeProposer` (9) | REJECT | `EnvelopePartialSig` (10); 1. Dissemination messages: 1 per (`MessageID`, slot) | proposal slot | none | 3 slots | proposer assignment at slot (IGNORE) | `SLOTS_PER_EPOCH` (IGNORE) |
...
- **EnvelopeProposer (9).**
  - *Dissemination class:* `EnvelopeDissemination` messages share the row's slot, lateness, assignment, and duty-limit rules. Structural rules: exactly one signer (REJECT above), SSZ decode into the container must succeed with size bounded by its SSZ maximum (REJECT otherwise). Dedup is first-valid: the first message per (`MessageID`, slot) passing all other rules is eligible for ACCEPT and MUST be recorded; once one is recorded, further dissemination messages for that tuple are IGNORE'd regardless of content or peer. An honest origin retry after the recipient's gossip duplicate cache expires can repeat a message, so repetition does not prove peer fault. The [§6](#6-new-duty-envelope-signing-self-build-path) checks binding the envelope to the decided block are runner concerns, not validation rules: validation decodes only the container structure and never judges the envelope's content.
  ...
  - *Window:* the duty is consumed within its own slot (the reveal is due at the `PAYLOAD_DUE_BPS` cutoff), so the short non-committee TTL applies and there is no earliness allowance.
...
|---|---|---|
| Role in {7, 8, 9} and `epoch(msg.slot) < GLOAS_FORK_EPOCH` | REJECT | These duties do not exist before the fork. The gate covers every message class these roles admit, including [§6](#6-new-duty-envelope-signing-self-build-path)'s dissemination messages. [§5](#5-proposer-preferences-duty)'s required pre-fork emission carries post-fork `proposal_slot` values by construction, so it passes this gate with no special case. |
...
The slot's envelope is missed if the builder operator fails to disseminate or publish, or if the [§6](#6-new-duty-envelope-signing-self-build-path) flow completes after `get_payload_due_ms()` so the signed envelope reaches PTC validators too late to observe before the cutoff. Publication is necessarily singular: only the builder operator holds the payload bytes, so there is no multi-publisher redundancy behind it. In either case PTC records `payload_present = FALSE` ([§3](#3-new-duty-payload-timeliness-committee-ptc-attestation)); the proposer forfeits the payload reward. No worse than the no-envelope-signing baseline for the self-build path.

### A forged dissemination can cost a self-build reveal, never a wrong payload

A `BlindedExecutionPayloadEnvelope` is cheap to forge: the four bound fields are public once the [§4](#4-modified-proposer-duty) block is, and `PayloadRoot` is the one field the [§6](#6-new-duty-envelope-signing-self-build-path) checks cannot verify, so a fabricated dissemination can pass them. Safety holds regardless: a publishable envelope needs the payload bytes behind its `PayloadRoot`, and fabricating a root with a payload behind it is a hash preimage, so only the builder operator can ever publish and a forgery can never put a wrong payload on chain. Under the first-valid rule, the residual is liveness: a Byzantine committee member that races its forgery ahead of the honest dissemination makes the cluster sign the wrong root, and the slot's reveal is missed. The trigger is narrow (self-build path only, an intentionally Byzantine operator inside the committee) and the loss is the bounded, non-slashable miss above, so this SIP accepts it, the same ship-simple call as PTC's convergence miss. A known hardening exists if the trade worsens: sign every valid dissemination, deduplicated per originator, so the honest root always reaches threshold; it is safe because the envelope signature (`DOMAIN_BEACON_BUILDER`) appears in no Gloas slashing predicate, making multiple signed envelope roots per slot non-slashable.
