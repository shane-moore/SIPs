...
- [Validator time parameters](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/validator.md#time-parameters)
...
- [Validator attestation changes](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/validator.md#attestation)
- [Gloas `Attestation` progressive container](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/beacon-chain.md#attestation)
...
The [Gloas same-slot rule](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/validator.md#attestation) (`block.slot == data.slot ⇒ data.index = 0`) has no beacon-node call on the value-check path: the cluster has only the QBFT-decided `BlockRoot` and trusts `AttestationDataIndex` from the leader. An operator SHOULD reject a value with `AttestationDataIndex = 1` when its own view already establishes that the block at `BlockRoot` has slot `duty.slot` (for example from a head event naming that root and slot), so the round changes instead of the cluster signing an attestation the network rejects. The check has no false positive: a reorg never changes a block's slot, and an honest beacon node never yields `index = 1` for a same-slot block. It MUST be evaluated against knowledge fixed when the instance starts and never re-evaluated with later knowledge, because QBFT re-runs the value check on justified re-proposals and decided values, and an operator that learned the block's slot mid-instance would reject a value it had already prepared. Operators without such a view skip the check. A bad same-slot `index=1` that escapes it is rejected by the ethereum network and ignored on chain but is not slashable, while cross-`index` equivocation over the same `(source, target, slot, BlockRoot)` is still caught by `IsAttestationSlashable` per the previous bullet.
...
- [Validator payload timeliness attestation flow](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/validator.md#payload-timeliness-attestation)
- [Beacon-chain payload attestation containers](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/beacon-chain.md#payloadattestationdata)
- [Fork-choice payload attestation deadline](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/fork-choice.md#new-get_payload_attestation_due_ms)
...
- [Validator block and sidecar proposal flow](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/validator.md#block-and-sidecar-proposal)
- [Gloas `BeaconBlockBody` progressive container](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/beacon-chain.md#beaconblockbody)
...
**Value check.** `ProposerValueCheckF` branches on the fork scheduled at `duty.Slot` before [`ProposerConsensusData.GetBlockData()`](https://github.com/ssvlabs/ssv-spec/blob/de34c611ce76bf6a40c3f94be0a44bde44d4a7ae/types/consensus_data.go#L192-L253), which stays Gloas-free. At Gloas slots it additionally rejects the value unless:

- the leader-supplied `Version` equals that fork;
- `duty.Slot` is the slot of the duty the operator is running, so that duty can complete;
- `block.slot == duty.Slot`, the slot the block is signed and slashing-protected under;
- `block.proposer_index == duty.ValidatorIndex`, whose key signs the block root; a mismatch would otherwise lose the slot at publication instead of changing round;
- `PayloadRoot == 0` iff `block.body.signed_execution_payload_bid.message.builder_index != BUILDER_INDEX_SELF_BUILD`.

An honest leader never trips these. Beyond presence, `PayloadRoot` is leader-trusted (see Security Considerations).
...
- [Broadcasting SignedProposerPreferences](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/validator.md#broadcasting-signedproposerpreferences)
- [`SignedProposerPreferences` container](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/p2p-interface.md#new-proposerpreferences)
- [`proposer_preferences` gossip topic](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/p2p-interface.md#new-proposer_preferences)
- [`execution_payload_bid` gossip validation](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/p2p-interface.md#new-execution_payload_bid)

Under Gloas, each proposer broadcasts `SignedProposerPreferences` on the `proposer_preferences` p2p topic for future proposal slots within the proposer lookahead (the current epoch up to `MIN_SEED_LOOKAHEAD` epochs ahead). The signed `ProposerPreferences` carries `dependent_root`, `proposal_slot`, `validator_index`, `fee_recipient`, and `target_gas_limit`. `dependent_root` pins the proposer-lookahead epoch's seed via `get_shuffling_dependent_root(store, root, epoch)`; operators populate it from the `dependent_root` returned by [`GET /eth/v2/validator/duties/proposer/{epoch}`](https://github.com/ethereum/beacon-APIs/blob/v5.0.0-alpha.2/apis/validator/duties/proposer.v2.yaml) for the proposal-slot's epoch. The slot bound is enforced at REJECT severity: [`proposer_preferences` gossip validation](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/p2p-interface.md#new-proposer_preferences) requires the `dependent_root` block to be at or before `compute_shuffling_dependent_slot(proposal_epoch)`, the last slot before the lookahead epoch, clamped to `GENESIS_SLOT`. A newer block is REJECT'd; a root failing the separate possible-dependent-block check is IGNORE'd. Builders listen to this topic and use a proposer's preferences to construct `execution_payload_bid` objects for that proposer's slots. This replaces the pre-Gloas out-of-band relay-registration mechanism, which is gone along with blinded blocks; the `ValidatorRegistration` duty is deprecated accordingly (end of this section).
...
Trigger: once per epoch for each epoch in the proposer lookahead, and again on duty-dependent-root changes for any epoch in the lookahead, iterate local validators and emit one duty per slot returned by `get_upcoming_proposal_slots(state, validator_index)`. An epoch's `dependent_root` block is one slot old at the boundary of the epoch before it and exposed to a late-block reorg there, so emitters SHOULD NOT emit an epoch's preferences before slot `SLOTS_PER_EPOCH // 2 - 1` of the epoch preceding it; by then the block has settled and builders still have over half an epoch of lead. Skew between operators does not affect correctness: receivers MUST retain preference and request-auth partials that arrive before their own duty for that proposal slot has started, apply them when it starts, and keep collecting until the proposal slot, so operators emitting at different times within the [§7](#7-ssv-message-validation) earliness window still reach threshold. In the `MIN_SEED_LOOKAHEAD` epochs immediately before `GLOAS_FORK_EPOCH`, this SIP requires operators to emit preferences for any local-validator proposal slots in the first Gloas epoch, under the same slot bound. Emission during the first Gloas epoch itself would also be gossip-valid for slots later in that epoch, but slots early in the epoch leave effectively no post-fork emission time, and builders need a validator's preference before they can construct and gossip bids for its slot; pre-fork emission covers both, aligning with the spec's *"Proposers SHOULD broadcast their preferences in the epoch before the fork"* recommendation in `p2p-interface.md`. The `proposer_preferences` gossip topic accepts only the first valid message per `(dependent_root, proposal_slot)` key (`validator_index` was dropped from the key upstream; the proposer check already determines it from the other two fields); emission-timing implications are covered in Security Considerations. If the proposer lookahead for an epoch changes, or `dependent_root` changes for an epoch already in the lookahead, cached duties for that epoch are replaced and a new preference is emitted. Because `dependent_root` is part of the gossip key above, the new preference is a distinct key rather than a replacement of the prior one. After a validator-set change (local validators registering, shifting indices), emitters SHOULD additionally delay (re-)emission by a couple of slots so committee peers converge on the new set before the one-shot partials go out; [§7](#7-ssv-message-validation) specifies the matching receive-side tolerance that keeps honest partials valid during the convergence window.
...
- [`DOMAIN_BUILDER_REQUEST_AUTH` constant](https://github.com/ethereum/builder-specs/blob/61aeca42d9ba8eb467ae230d191ebc1c3f38107d/specs/gloas/builder.md#constants) and [signing functions](https://github.com/ethereum/builder-specs/blob/61aeca42d9ba8eb467ae230d191ebc1c3f38107d/specs/gloas/builder.md#signing)
- [Constructing the `BuilderRequestAuth`](https://github.com/ethereum/builder-specs/blob/61aeca42d9ba8eb467ae230d191ebc1c3f38107d/specs/gloas/validator.md#constructing-the-builderrequestauth)
...
Direct builder connections ([§4](#4-modified-proposer-duty)) authenticate each request with `SignedBuilderRequestAuth`: a validator-key BLS signature over `BuilderRequestAuth { data: ByteList[4096], slot }` under `DOMAIN_BUILDER_REQUEST_AUTH` (`0x0B000001`), computed with `compute_domain` defaults (genesis fork version, zeroed `genesis_validators_root`), the same chain-independent construction as the deprecated `ValidatorRegistrationV1` and one byte from [§6](#6-envelope-signing-self-build-path)'s `DOMAIN_BEACON_BUILDER` (`0x0B000000`). `slot` is the proposal slot the request is authorized for, never the slot at which it is signed or sent, so it is a pure function of the proposer lookahead.

`data` is the byte string agreed with the builder out of band, signed exactly as configured. When nothing is agreed, it defaults to the result of the upstream [`get_default_auth_data`](https://github.com/ethereum/builder-specs/blob/61aeca42d9ba8eb467ae230d191ebc1c3f38107d/specs/gloas/validator.md#default-auth-data) rule: the lowercase ASCII hostname, excluding scheme, userinfo, port, path, query and fragment. IPv6 literals use the upstream compressed, bracketed form with hexadecimal groups only, never the mixed IPv4 notation; zoned IPv6 literals are invalid builder configuration; internationalized hostnames must be supplied in punycode. The upstream example table is the conformance set, and implementations MUST reproduce its `data` bytes exactly, because standard-library URL and address formatters differ across languages on lowercasing, zone decoding and IPv4-mapped output. This derivation changes neither the routing URL nor explicitly configured auth bytes, and no intermediary rewrites the signed `data`. Zero-length `data` is invalid, not a request to use the default. One signature covers a `(data, proposal_slot)` pair, and entries sharing resolved `data` share it; builders needing distinct identities on one hostname agree explicit `data` instead.

The signing round rides the proposer-preferences duty: same runner and role, same triggers, same `proposal_slot` message slot, with partial signatures typed `RequestAuthPartialSig` instead of `ProposerPreferencesPartialSig`. No new beacon role, runner role, or `MapDutyToRunnerRole` entry is added. For each configured builder entry of an upcoming proposal slot, each operator derives `BuilderRequestAuth{data, proposal_slot}`, signs it with the validator's BLS share key, and broadcasts the partials in one or more `PartialSignatureMessages` containers of 1 to 8 entries each. The duty runner matches incoming partials against the root set derived from its own configured entries and collects per root; unmatched roots MUST NOT prevent collection of matching roots in the same packet. This matching is not a pubsub validation rule. At most one share per operator counts per root, independently of the preference round and of other roots; a root reaching the reconstruction threshold yields one `SignedBuilderRequestAuth`, held for the proposal slot. Auth roots contain no `dependent_root`, so the re-emission triggers above reproduce byte-identical roots: an operator broadcasts each root's partial once per emission (an honest retry may repeat it; [§7](#7-ssv-message-validation) deduplicates by distinct root). The preference flow is unchanged, and neither round gates the other. Deployment note: the `RequestAuthPartialSig` wire variant must be decodable network-wide before any operator emits it, because a container with an unknown partial-signature type fails structural decoding and is REJECT'd, penalizing the forwarding peer, unlike the IGNORE-classed budget rules of [§7](#7-ssv-message-validation). Receivers MUST admit multi-entry auth packets network-wide before any operator emits them; singleton emission remains valid.
...
- [`ExecutionPayloadEnvelope` container](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/beacon-chain.md#executionpayloadenvelope)
- [EIP-7495 `ProgressiveContainer` merkleization](https://eips.ethereum.org/EIPS/eip-7495#merkleization)
- [`execution_payload` gossip topic (carries `SignedExecutionPayloadEnvelope`)](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/p2p-interface.md#new-execution_payload)
- [`POST /eth/v1/beacon/execution_payload_envelopes` endpoint](https://github.com/ethereum/beacon-APIs/pull/624)

On the self-build path (`bid.builder_index == BUILDER_INDEX_SELF_BUILD` per [EIP-7732](https://eips.ethereum.org/EIPS/eip-7732)), the proposer signs `SignedExecutionPayloadEnvelope` alongside the block. The [§4](#4-modified-proposer-duty) decision pins exactly one valid envelope: `bid.block_hash` commits to the whole payload, `bid.execution_requests_root` commits to the requests, and [`verify_execution_payload_envelope`](https://github.com/ethereum/consensus-specs/blob/477321355d48d527e7e1e4d572f6a40a0b41072a/specs/gloas/fork-choice.md#new-verify_execution_payload_envelope) checks every envelope field against the committed bid and block. With `PayloadRoot` in the decided value, every field of the envelope's signing input is derivable from that value, so the envelope signature rides the [§4](#4-modified-proposer-duty) post-consensus packet.
...
| `RoleProposerPreferences` (8) | REJECT | `ProposerPreferencesPartialSig` (8); up to 4 distinct signing roots. `RequestAuthPartialSig` (9); up to 8 distinct signing roots | `proposal_slot` | from the start of epoch `epoch(proposal_slot) - MIN_SEED_LOOKAHEAD` | 2 slots | proposer assignment at `proposal_slot` (IGNORE) | `SLOTS_PER_EPOCH` (IGNORE) |
...
- **ProposerPreferences (8).**
  - *Earliness:* the message slot is `proposal_slot` ([§5](#5-proposer-preferences-duty)), up to the proposer lookahead in the future at emission. The allowance opens when the proposer becomes known, which is also when gossip starts accepting the reconstructed preference: an earlier partial could only reconstruct a preference no node propagates, and the assignment check cannot cover it.
  ...
  - *Request-auth dedup:* `RequestAuthPartialSig` has a separate budget of 8 distinct roots per (`MessageID`, signer, `proposal_slot`). Repeated roots within a packet count once toward this budget, but each entry counts toward the raw 8-entry limit. If this rule is the only failing condition, a packet MUST receive IGNORE when it adds no new root or when the union of its roots and previously recorded roots exceeds 8. Otherwise it is eligible for ACCEPT, including when it mixes recorded and new roots, subject to all other validation rules. On ACCEPT, implementations MUST record all new roots together; IGNORE or REJECT records none. Budgets are tracked per partial-signature type; neither consumes the other. Honest re-triggers reproduce identical roots by design ([§5](#5-proposer-preferences-duty)). Type-9 messages ride existing duty slots and add no distinct slots to the per-epoch duty limit.
...
Fork gates, a new rule class. The slot-scoped rules are REJECT: the gated condition is carried in the message itself, so no honest ordering race can produce it. The wall-clock-scoped rule is IGNORE, since it depends on the receiver's clock.
...
| `RoleValidatorRegistration` (4) received at wall-clock epoch `> GLOAS_FORK_EPOCH` | IGNORE | Registrations carry no slot deadline, so one stamped with a pre-fork slot but arriving after the fork passes the row above and can only be a replay. The fork epoch itself still admits partials in flight from the last pre-fork slots. |
...
### Builder `data` divergence can prevent auth reconstruction

`BuilderRequestAuth.data` is per-builder configuration, not an observation: different resolved bytes split signing roots, for example when some operators use the hostname default and others still use the full URL. If no root reaches threshold, no auth signature reconstructs for that builder ([§5](#5-proposer-preferences-duty)); one divergent operator alone need not prevent reconstruction, but reduces fault tolerance. Hostname case and a trailing URL slash do not change the default auth bytes. Per-root matching and collection bound the damage to the affected roots; other entries, p2p bids, and self-build are unaffected. The cluster-wide configuration requirement in §5 still applies.
