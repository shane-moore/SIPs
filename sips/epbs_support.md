...
- **Envelope signing is dissemination plus threshold signing, not consensus.** Once [§4](#4-modified-proposer-duty) decides, `bid.block_hash` pins exactly one valid envelope, so there is no value to negotiate; consensus would only add message phases and a leader schedule inside the tightest window in this SIP. [§6](#6-new-duty-envelope-signing-self-build-path) instead disseminates a `BlindedExecutionPayloadEnvelope` (`payload` → `payload_root: Root`), whose progressive hash tree root equals the full envelope's, and threshold-signs its root, keeping messages bounded (~few hundred bytes vs hundreds of KB to ~MB).
...
Fires after the [§4](#4-modified-proposer-duty) self-build block is signed and published; the trigger carries the decided block. There is no beacon-node duty fetch.
...
`PayloadRoot` has no local check: it is trusted from the builder operator, matching the existing blinded-block trust model in `BNRoleProposer`, where operators do no field-level validation against their local BN view. A fabricated `PayloadRoot` cannot yield a publishable envelope (see Security Considerations).
